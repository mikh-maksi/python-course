...
**Исходный код (source code)** &mdash; набор фраз, слов, специальных символов и т. д., специфичных для языка программирования, описывающих набор инструкций для компьютера.  
...
Отсюда можно сделать вывод &mdash; код пишется не для машины, а для разработчика. Исходный код должен не только верно решать задачу, но и иметь смысл, быть понятным и легко читаемым. Это одна из самых сложных задач для начинающих (и не только), на ее решение уйдет много времени и сил.  
...
Опытные разработчики рассматривают проблемы с точки зрения алгоритмов &mdash; набора шагов, которые нужно выполнить для достижения определённой цели, даже если детали меняются.
...
## Python
**Python** &mdash; высокоуровневый язык программирования общего назначения, ориентированный на повышение производительности разработчика и читаемости кода. Синтаксис ядра Python минималистичен. В то же время стандартная библиотека включает большой набор полезных функций.
...
Например, `2 + 3` &mdash; это выражение. Выражения в Python отделяются другого от друга знаком переноса на новую строку.
Проще говоря, одна строка &mdash; одно выражение. Выражения можно отделять друг от друга символом `;`, но такой 
синтаксис нежелателен, хотя и не вызывает ошибки.

Python выполняет выражения одно за другим по очереди слева направо и сверху вниз. 
...
## Переменные
Переменная &mdash; это имя (псевдоним) для некотрой области памяти компъютера. Предполагается, что в этой памяти лежит
...
2. Числа (Numeric Type)
    - `int` &mdash; целое число
        - `bool` &mdash; подтип `int`, который может принимать только два значения (`True` или `False`).
    - `float` &mdash; число с плавающей точкой
    - `complex` &mdash; комплексное число

3. Строки (Text Sequence Type)
    - `str` &mdash; обычные строковые переменные в любой удобной кодировке (UTF-8 по умолчанию).
    - `bytes` &mdash; байт-строки, строковые переменные где для хранения одного символа используется строго один байт (кодировка ASCII).
    - `bytearray` &mdash; изменяемые байт-строки, дают возможность работать с такой строкой как со списком байтов.

4. Списки (Sequence Type)
    - `list` &mdash; список, контейнер в котором упорядоченно хранятся любые данные.
    - `tuple` &mdash; кортеж, тоже что и список, но неизменяемый.

7. Множества (Set Types)
    - `set` &mdash; множество
    - `frozenset` &mdash; неизменяемое множество
...
Строковые переменные &mdash; это упорядоченные неизменяемые наборы символов. "Упорядоченные" означает, что можно обращаться к символам
...
ввести имя этой функции в окне интерпретатора и аргументы в круглых скобках. Например, выведем строку "Hello, World!":
...
- float &mdash; вещественные числа (числа с плавающей точкой)
- complex &mdash; комплексное число
...
>**Функция** (в математике) &mdash; это соответствие вида y = f(x) между переменными величинами, в силу которого каждому рассматриваемому значению некоторой переменной величины x (аргумента или независимой переменной) соответствует определенное значение другой переменной величины, y (зависимой переменной, иногда это значение просто называют значением функции).
...
## Логический тип
Логический тип (boolean) &mdash; это подтип `int`, целых чисел, который может принимать только 2 значения: 
True (истина) или False (ложь). 
Наиболее близкий пример из повседневной жизни &mdash; это принятие решения да/нет (идти/не идти, 
...
## Определение типов
**Python** &mdash; язык с динамической типизацией. Т.е. интерпретатор сам назначает тот тип, 
...
```
Динамическая типизация дает возможность писать код очень гибко и не думать о работе с памятью на элементарном уровне.
...
В примере выше ярко представлена суть "динамической" типизации, мы создаем переменную `a`, которая является строковой
переменной и содержит строку "qwe". Уже на третей строке кода `a` указывает на область памяти где хранится число 4.
...
|Оператор|Значение|Примеры|
|---|---|---|
|in|Если элемент присутствует в последовательности - возвращает True<br>В противном случае &mdash; 
...
|not in|Если элемент присутствует в последовательности - возвращает False<br>В противном случае &mdash;
...
|Оператор|Значение|Примеры|
|---|---|---|
|is|Если два операнда указывают на один и тот же объект &mdash; вернет True|x is y вернет True, если id(x) равно id(y)|
|not is|сли два не операнда указывают на разные объекты &mdash; вернет False|x is not y вернет True, если id(x) не равно id(y)|
...
имя у польщователя и после ввода имени выводить сообщение `Hello {name}`. 
