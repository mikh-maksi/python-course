...
Коментарии в Python обозначаются символом `#`, всё что после этого символа и до начала новой строки интерпритатор
...
Числа в Python представленны целыми, дробными и комплексными числами. Python динамический язык, а значит
при создании переменной не нужно определять её тип. Интерпритатор сам определит к какому типу она относится исходя из
её значения и определит сколько памяти для неё выделить.

В примере выше инструкция `int_number = 3` создаст переменную **int_number** со значением **3** и **int_number** буде 
типа `int`.
...
Синтаксичкски f-строка отличается от обычной тем, что в начале строки стоит символ `f`. Интерпритатор поймет, что если в
такой строке он встретит символы фигурных скобок `{}`, то внутри них сожержится выражение, которое надо выполнить и
...
Стандартная бибилиотека Python реализует ряд функций, которые сразу доступны разработчику без необходимости их как-то
...
`print` удобно испольщовать узнать содержимое той или иной переменной по мере выполнения инструкций.
...
- Запустить скрипт `hello.py` с помощью python интерпритатора.
