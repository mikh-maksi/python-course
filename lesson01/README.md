...
**Python** - имеет динамическую типизацию, т.е. программа-компилятор сама определяет тип переменной. 
При необходимости тип данных можно изменить, используя функии приведения типов:

- `int` преобразует аргумент в целое число
- `str` преобразует аргумент в строку
- `float` преобразует аргумент в дробное десятичное число.
- `bool` преобразует аргумент в `True` или `False`.

Кроме явного приведения типов Python может делать приведение неявно, например результат деления двух чисел всегда 
будет `float`, а арифметика с комплексным числом, всегда даст комплексный результат.

Но неявное приведение считается примером "плохого" кода и на практике встречается не часто.
