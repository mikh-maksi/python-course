...
Также, общим для всех контейнеров Python есть возможность перебрать все элементы контейнера в цикле или 
...
* Сохраните в словаре `alphabet` символы из `string.ascii_letters` (список букв английского
    алфавита). В качестве ключа используйте букву, а значение &mdash; её порядковый номер в `string.ascii_letters`.
...
* `unique_words` возвращает кортеж из двух множеств. Первое &mdash; это общие слова для `text1` и `text2`,
    второе &mdash; это уникальные слова, которые есть только в `text1` или только в `text2`.
